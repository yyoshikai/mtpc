...
class VICRegLTransform(object):
    def __init__(
        self,
        example_dir, n_example, 
        size_crops=[224, 96],
        num_crops=[2, 6],
        ...
        for i in range(len(size_crops)):
            for j in range(num_crops[i]):
                ...
                self.augmentations.append(get_color_distortion(left=(j % 2 == 0)))
    ...
    def __call__(self, img: Image):
        ...
        for i, (crop, location) in enumerate(multi_crops_no_augs):
            ...
            crop_image = self.augmentations[i](crop)
            multi_crop_images.append(crop_image)
            multi_crops.append(self.tensor_transform(crop_image))
            grid_size = 7
        ...
        if self.n_saved < self.n_example:
            os.makedirs(f"{self.example_dir}/{self.n_saved}/x", exist_ok=True)
            img.save(f"{self.example_dir}/{self.n_saved}/original.png")
            for i, crop_image in enumerate(multi_crop_images):
                crop_image.save(f"{self.example_dir}/{self.n_saved}/{i}.png")
            self.n_saved += 1

        return {'views': multi_crops, 'locations': locations}
...
class RandomResizedCropWithLocation(torch.nn.Module):
    ...
    Args:
        ...
        interpolation (InterpolationMode): Desired interpolation enum defined by
            :class:`torchvision.T.InterpolationMode`. Default is ``InterpolationMode.BILINEAR``.
